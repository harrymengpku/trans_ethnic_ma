MarkerNameCheck <- function(input, marker="MARKERNAME", chr="CHROMOSOME", pos="POSITION",markercheck=FALSE) {
# For Meta-analysis, check if have geniune markername, chr and pos
# valid chr must between 1 and 23
# valine pos much >0
...
check_chr_2 <- (!is.na(input[[chr]])) & input[[chr]] %in% c(1:23)
...
ba <- c("A","C","G","T")
check_chr_1 <- (!is.na(input[["chr.new"]])) & input[["chr.new"]] %in% c(1:23)
